...
**Key:** dashed arrows inside the gateway are the fallback order — if Provider A fails, is too slow, or gets too expensive, the router falls back to B, then C, without any capability needing to know or care which provider actually answered.
